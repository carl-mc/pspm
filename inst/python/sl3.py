...
## Cracking
class VertexDismembermentNS(NetworkStatistic):
    # NS that evaluates how dismembered groups are
    groups: List
    weights: List
    stat: List
...
## Packing - fast
class VertexSuffocationNS(NetworkStatistic):
    # NS that evaluates how suffocated groups are 
    groups: List
    weights: List
    stat: List
    ...
    def compute_statistic(self) -> None:
        ...
        for c in range(self.spatial_lattice.P):
          ...
          res = []
          for i in gu:
            res.append(1 - hhi(x[self.partitioning == i]))
          self.stat.append(np.array(res))
          self.statistic[c] = sum(self.stat[c] * self.weights[c])
...
class PartitionModel:
    ...
    def __init__(self, spatial_lattice: SpatialLattice, partitioning: List[int],
        ...
        if "VertexCrackNS" in net_stats:
          vertex_crack_ns: VertexCrackNS = VertexCrackNS(spatial_lattice, partitioning)
          self.network_statistics[vertex_crack_ns.get_name()] = vertex_crack_ns
        
        if "VertexPackNS" in net_stats:
          vertex_pack_ns: VertexPackNS = VertexPackNS(spatial_lattice, partitioning)
          self.network_statistics[vertex_pack_ns.get_name()] = vertex_pack_ns
      
        if "VertexDismembermentNS" in net_stats:
          vertex_dismemberment_ns: VertexDismembermentNS = VertexDismembermentNS(spatial_lattice, partitioning)
          self.network_statistics[vertex_dismemberment_ns.get_name()] = vertex_dismemberment_ns
        
        if "VertexSuffocationNS" in net_stats:
          vertex_suffocation_ns: VertexSuffocationNS = VertexSuffocationNS(spatial_lattice, partitioning)
          self.network_statistics[vertex_suffocation_ns.get_name()] = vertex_suffocation_ns
