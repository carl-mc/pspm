...
the pspm package heavily builds on the igraph library which is used to handle the underlying netowrk data and allows user-friendly data manipulation. Since random sampling plays an important role during sampling, it is recommended to set a random seed in R and python. `pspm_set_seed()` achieves just that. 
